...
export default function ProjetosAndamento() {
  ...
  async function carregarProjetos(userId) {
    ...
    if (error) {
      console.error(error);
      alert("Erro ao carregar projetos em andamento");
    } else {
      setProjetos(data || []);
    }
  ...
  async function finalizarProjeto(id) {
    const confirmar = confirm("Deseja marcar este projeto como finalizado?");
    if (!confirmar) return;
  ...
  return (
    <main style={{ padding: 30, maxWidth: 1000, margin: "0 auto" }}>
      ...
      <p>Acompanhe os projetos que você aceitou e está executando.</p>

      {projetos.length === 0 && (
        <div style={card}>
          <p>Você ainda não possui projetos em andamento.</p>
        </div>
      )}
      ...
      {projetos.map((item) => (
        <div key={item.id} style={card}>
          ...
          {item.status === "em_andamento" && (
            <button style={button} onClick={() => finalizarProjeto(item.id)}>
              Finalizar projeto
            </button>
          )}

          {item.status === "finalizado_freela" && (
            <p style={{ color: "#b7791f", fontWeight: "bold" }}>
